...
def candidates_generator(qids, embedddings, root, node_dict, k, model):  #qid
    ...
    while Q:
        ...
        probs = []
        for node in Q:
            prob = model.predict(qids, embedddings, node)
            print("okkk")
            print(prob)
            return 0
            probs.append(prob[0])
    ...
    probs = []
    for leaf in A:
        prob = model.predict(qids,embedddings,leaf)
        probs.append(prob[0])
...
def metrics_count(data, embeddings, root, node_dict, k, model):   #(vtest, tree.root, 10, model
    # for items in data:
    rank_list = []
    for qids in data:
        size = qids.shape[0]  #行
        for i in range(size):
            cands = candidates_generator(qids[i], embeddings[i], root, node_dict, k, model)  #返回节点
            for j in range(k):
